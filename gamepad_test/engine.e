...
feature -- Access
	...
	run
		...
		do
			...
			game_library.findcontroller
		--	io.put_string ({GAME_SDL_EXTERNAL}.sdl_numjoysticks.out+ " joysticks    ")
			if attached game_library.internal_gamepads.first as l_controller then
...
feature {NONE} -- Implementation
	...
	on_iteration(a_timestamp:NATURAL_32)
		...
		do
			...
			game_library.update_gamepads_state
	...
	local
		l_int: INTEGER
		do
		--	if a_button_id.is_equal (controller.south_button.as_natural_8) then
				maryo.go_right(a_timestamp)

				io.put_string (	controller.get_axis(controller.axis.trigger_left).out)
		--	end
	...
	on_button_released(a_timestamp:NATURAL_32; a_button_id:NATURAL_8)
		do
		--	if a_button_id.is_equal (controller.dpad_left_button.as_natural_8) then
				maryo.stop_right
		--		io.put_string ("what the hell")
		--	end
		--	io.put_string (a_button_id.out)
		end
